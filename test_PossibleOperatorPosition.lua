--
-- тестирование  PossibleOperatorPosition  (май 2021)
--
...
--
function Calc_POP( ) -- вычисление интенсивности PossibleOperatorPosition
local nTiers=GetCountTiers() -- общее число ярусов (без нулевого яруса)
local nOps=GetCountOps()  -- общее число операторов (без нулевого яруса)
--
for iGap=0,nTiers-1 do -- по всеми промежуткам между ярусами ( iGap - верхний ярус промежутка )
--
OutLine(  "\n==== iGap= "  .. iGap .. " ====="  )
--
sumPOP = 0 -- число возможных перемещений
 for iOp=1,nOps do -- по всем операторам
 Op = GetNumbOp( iOp ) -- номер оператора
--
 minTier = GetMinTierMaybeOp( Op )
 maxTier =  GetMaxTierMaybeOp( Op )   -- если равны - не используем !!!
--
-- if GetTierByOp( Op ) >= iGap+1 and minTier = maxTier then -- если яруса оператора ниже, его рассматривать не будем !!!
--  goto no_sum 
-- end 
--
OutLine( "==== Op= " .. Op .. "   "  ..  minTier .. " - " .. maxTier  .. "  Tier= " ..  GetTierByOp( Op ) )
--
 if minTier >= iGap and maxTier <= iGap then -- iGap находится внутри диапазона minTier-maxTier
  sumPOP = sumPOP + 1 -- суммируем
 end -- конец if
--
::no_sum:: -- операторов не рассматривается
--
 end  -- конец  for iOp=1,nOps do
--
OutLine(  "iGap="  .. iGap .. " | " ..  iGap+1 .. "   " .. "sumPOP= " .. sumPOP )
--
SoundPlay("kuku.wav")
-- 
end -- конец for iGap=1,nTiers-1 do   
--
end -- конец функции  Calc_POP  -----------------------------------------------------------------------
--
-- MAIN - функция -------------------------------------
--===================================
--
