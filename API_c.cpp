...
{ // преобразует FileName в нужную форму с учЄтом необходимого расширени€ Ext (с точкой  в начале)
//
 char NewFileName[_1024];
//
 strcpy( NewFileName, FileName ); // им€ файла запомнили в NewFileName
 ChangeFileExt( NewFileName, Ext ); // изменили на ".Ext"
//
 if( !FileExists( NewFileName ) ) // файл не существует...
 {
  DisplayMessage( "W", __FUNC__,
                   Format("‘айл %s не существует (исходное им€: %s)",OPENARRAY(TVarRec,(NewFileName,FileName)) ).c_str(),
                   ERR_UNCERTAIN );
  return "''None''"; // недопустимое им€ файла
 }
//
 return NewFileName ;
//
