...
//
     DelSpacesTabsAround( s_ret ); // удал€ем внешние пробелы
...
//
// если ни одного подпараметра, то дезавуируем
// [=][число]/[число][:] или [=][Def:] в строке sOps
//
 if( countSP==0 )
  sOps[ lPrevCalcs ] = '\0' ;
//
...
//
 DelSpacesTabsAround ( sCalcs ); // удал€ем внешние пробелы
 ReplManySpacesOne( sCalcs ); // удал€ем повторы пробелов внутри
//
#ifdef TEST_PRINT //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//
 DelSpacesTabsAround ( sOps ); // удал€ем внешние пробелы
 ReplManySpacesOne( sOps ); // удал€ем повторы пробелов внутри
//
#ifdef TEST_PRINT //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
// --- начало обработки строки параметра sP ------------------------------------
//
//  snprintf( s,sizeof(s), "sP (%d-%d) |%s|", jStart, jEnd, sP ) ;
//  c_AddLineToTextFrame( s );
//  MessageBeep( MB_OK );
//  Delay( 100 );
//
// --- проверка на корректность считывани€ значений параметров путЄм использовани€ sscanf(...)
...
//
 DelSpacesTabsAround ( sEdg ); // удал€ем внешние пробелы
 ReplManySpacesOne( sEdg ); // удал€ем повторы пробелов внутри
//
#ifdef TEST_PRINT //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//
 DelSpacesTabsAround ( sVrt ); // удал€ем внешние пробелы
 ReplManySpacesOne( sVrt ); // удал€ем повторы пробелов внутри
//
#ifdef TEST_PRINT //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
//
  ...
  {
   DelSpacesTabsAround( sR ); // удал€ем пробелы спереди и сзади
...
//
//  if( sscanf( sN, "=%d/%d|%d/%d: ", &n1,&n2, &m1,&m2 ) == 4 ) // правильное чтение - 4 пол€ прочитано
...
//
 strNcpy( FileName, ChangeFileExt( FileName, extEdges ).c_str() ); // расширение - extEdges
//
 if( !(fptr = fopen(FileName, "r")) ) // файл описани€ графа открыли дл€ чтени€
 {
...
//
 strNcpy( FileNameEdges, FileName ); // запомнили текущее им€ файла описани€ »√ј
//
...
//
  ReplManySpacesOne( str ); // замен€ем множество пробелов внутри строки одним-единственным
  DelSpacesTabsAround( str ); // удал€ем пробелы в начале и конце строки
//
...
//
   DelSpacesTabsAround( Mem_Edges[nEdges].Operators ); // удал€ем пробелы справа и слева в строке Mem_Edges[nEdges].Operators
   ReplManySpacesOne( Mem_Edges[nEdges].Operators ); // множествкнные пробелы замен€ем единственным
//
...
//
 DelAllSpaces( str ); // удал€ем ¬—≈ пробелы в строке str
//
...
//
 DelSpacesTabsAround( str ); // удал€ем пробелы в Ќј„јЋ≈ и конце строки
//
...
//
 for( INT j=0; j<strlen(sWork); j++ ) // по всей строке sWork...
  if( sWork[j] == '\n' || sWork[j] == '\r' || sWork[j] == '\t' )
   sWork[j] = ' ';
//
 DelSpacesTabsAround ( sWork ); // удал€ем внешние пробелы
 ReplManySpacesOne( sWork ); // удал€ем повторы пробелов внутри
//
 return TRUE;
...
//
 averTLD /= 1.0*n; // средн.арифметическое времени жизни данных между €русам
//
...
*/
 ...
 {
//  sprintf( NewFileName, "Ќедопустимое им€ файла: %s , файл переименован в: %s%s", // временное использование строки NewFileName...
//                         FileName,Unknown_Name,ExtractFileExt(FileName).c_str() );
//  DisplayMessage( "W", __FUNC__, NewFileName, ERR_UNCERTAIN ); // выдать сообщение
  TVarRec Vr[]={ FileName,Unknown_Name,ExtractFileExt(FileName).c_str() }; // открытый массив дл€ последующей печати
//  t_printf( "\n---%s---%d---\n", ExtractFileExt(FileName).c_str(), strlen(ExtractFileExt(FileName).c_str()) );
//  t_printf( TrimRight(Format("Ќедопустимое им€ файла: %s , файл переименован в: %s%s",Vr,2)).c_str() ); // .txt'верт.пр€моугольник' = TrimRight + c_str()
...
//
 DelSpacesTabsAround( FileName ); // удал€ем пробелы и Tabs с начала и с конца строки
...
//
 p = strtok(wF,Comma); // ищем первое вхождение '.' (в p будет всЄ, что левее '.')
 if( !p ) // если p пусто...
  strcpy( NewFileName,Unknown_Name ); // им€ собственно файда не определено
 else
  {
   strcpy( NewFileName,p ); // им€ собственно файла определено и дбавлена '.'
