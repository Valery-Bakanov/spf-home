...
#define SS_01 " \x00A6 " // строка-разделитель блоков при выводе в с_PutParamsTiers() // Special Sequence двойна€ вертикальна€
...
//
 ...
 {
  OpsOnTier = c_GetCountOpsOnTier( iTier ); // число операторов на на €руск
// --- устанавливаем цвета графика ---------------------------------------------
  ...
  { TIM1->Canvas->Brush->Color = brush_draw_color_TIERS; // цвет кисти обычный дл€ Tiers
...
//
////////////////////////////////////////////////////////////////////////////////
// ----- рисуем вертикальную линию - среднее значение ширин €русов -------------
...
// ищем мах TLD в диапазоне промежутков €русов 0/$ -----------------------------
  ...
  { maxCountTLD_0 = max(maxCountTLD_0,CountTLD);
    n1x_0=n1; n2x_0=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
// ищем мin TLD в диапазоне промежутков €русов 0/$ -----------------------------
  if( CountTLD < minCountTLD_0 )
  { minCountTLD_0 = min(minCountTLD_0,CountTLD);
    n1n_0=n1; n2n_0=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
//
...
// ищем мах TLD в диапазоне промежутков €русов 1/$ -----------------------------
  ...
  { maxCountTLD_1 = max(maxCountTLD_1,CountTLD);
    n1x_1=n1; n2x_1=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
// ищем мin TLD в диапазоне промежутков €русов 1/$ -----------------------------
  if( CountTLD < minCountTLD_1 )
  { minCountTLD_1 = min(minCountTLD_1,CountTLD);
    n1n_1=n1; n2n_1=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
//
...
//
 if( nTiers != 1 )
  ...
  snprintf( szStatTiers,sizeof(szStatTiers), "ср.арифм.шир.= %.4g, — ќ= x.x, CV= x.x, IC= x.x, ICL= x.x, DC= х.х",
            StatTiers.averWidth ) ;
//
// --- вычисл€ем ¬ј–»ј“»¬Ќќ—“№ -------------------------------------------------
...
//
 Delay( 100 ); // ждЄм 0,1 сек дл€ гарантированного дистижени€ уникальности нового имени файла
//
...
{ // преобразует FileName в нужную форму с учЄтом необходимого
// расширени€ Ext (с точкой в начале)
//
...
/* t_printf( ")%s(", FileName );
//
 if( FileName[strlen(FileName)-1] == '.' ) // если '.' последн€€...
...
////////////////////////////////////////////////////////////////////////////////
//
// дл€ экономии кода каркас вз€т из c_DrawDiagTiers(), а функциональна€ --------
...
//
 sscanf( paramsTLD->Strings[0].c_str(), "%d", &nGaps ); // общее число меж€русных промежутков в яѕ‘
//
...
//
  ...
  { maxCountTLD = max(maxCountTLD,CountTLD);
    n1x=n1; n2x=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
//
  if( CountTLD < minCountTLD ) // ищем мin число живых данных
  { minCountTLD = min(minCountTLD,CountTLD);
    n1n=n1; n2n=n2; } // запомнили €рус выше | запомнили €рус ниже (избыточно вообще-то...)
//
...
//
 snprintf(str,sizeof(str), "H|N/W=%d|%d/%d-%d", nGaps,maxCountTLD, n1x,n2x ); // число лент |/ мах данных / €рус_вверху-€рус_внизу
 F2->L_OM->Caption = str;
 F2->L_OM->Repaint(); // принудительно перерисовываем
//
 TIM1->Picture->Bitmap->Height = TIM1->Height; // настроить размеры —anvas по размерам Image
...
//
  ...
  TIM1->Canvas->Pen->Mode  = pmCopy; // цвет при взимодействии с фоном
  TIM1->Canvas->Pen->Style = psDot;  // точечна€ лини€
  TIM1->Canvas->Pen->Width = 1; // толщина пера 1 пиксел€
//
